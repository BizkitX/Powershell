...
do {
    ...
    if ($arrRunning.length -gt 0 ) {  
		...
		"`n"
		Write-Host "  ====> You can now open Internet Explorer" -ForegroundColor 'Magenta' 
		
Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Internet Explorer\Main\' -Name 'start page' -Value 'https://mottmac.sharepoint.com/Pages/Home.aspx'

"`n"
Write-Host  "  ============================================" -ForegroundColor 'Green'
Read-Host   "   Resetting Done. Press ENTER key to close" 
Write-Host  "  ============================================" -ForegroundColor 'Green'
"`n"
